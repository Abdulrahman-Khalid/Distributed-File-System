...
def MasterDK_Rep(dataKeepers, files_metadata):
    while True:
        for file in files_metadata.values():
            ...
            ip, port = get_source_Machine(file.DKs)
            ...
            for ipPort in freePorts:  
                src_socket ,src_context = configure_port(ip + ":" + port, zmq.REQ, 'connect')
                srcMessage = {'id': MsgDetails.MASTER_DK_REPLICATE ,"type":DataKeeperType.SRC}
                src_socket.send(pickle.dumps(srcMessage))  

                dst_socket, dst_context =configure_port(ipPort[0] + ":" + ipPort[1],zmq.REQ, 'connect')
                dstMessage={'id':MsgDetails.MASTER_DK_REPLICATE ,"type":DataKeeperType.DST, 'srcIp':ip, 'srcPort': port}
