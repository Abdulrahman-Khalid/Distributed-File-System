...
def DK_Rep(myPort, filePath, arrFullPaths):
    ...
    mainSocket, mainContext = configure_port("*:{}".format(myPort), zmq.REP, "bind")
    ...
    while (True):
        ...
        if(recievedMsg["id"] == MsgDetails.MASTER_DK_REPLICATE):
            if(recievedMsg["type"] == DataKeeperType.DST):
                replicate_as_DST(recievedMsg, filePath, arrFullPaths)
            elif(recievedMsg["type"] == DataKeeperType.SRC):
                replicate_as_SRC(recievedMsg, myPort, myIp, filePath)
            ...
            recieve_from_client(recievedMsg, filePath, arrFullPaths, myIp, myPort, mainSocket)

        

def replicate_as_DST(recievedMsg, filePath, arrFullPaths):
    # Connect to Source
    socket, context = configure_port(
    recievedMsg["srcIp"] + ":" + recievedMsg["srcPort"], zmq.PULL, "connect")
...
def replicate_as_SRC(recievedMsg, myPort, myIp, filePath):
    # Configure my port 
    socket, context = configure_port(
                myIp +":"+ myPort, zmq.PUSH, "bind")
